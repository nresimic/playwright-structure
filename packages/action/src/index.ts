...
function getIntensity(region: string, date: Date): number {
...
function readInput(name: string, def: string) {
  const v = process.env[`INPUT_${name.toUpperCase()}`];
  return v && v.length > 0 ? v : def;
}

async function measure(mode: string, apiUrl: string, region: string, threshold: number): Promise<{ intensity: number; clean: boolean; reason: string }> {
...
async function run() {
  ...
  while (true) {
    ...
    if (waited >= maxWait) {
      ...
      if (failOnTimeout) process.exit(1);
      return;
    }
